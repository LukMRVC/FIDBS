...
public:
    ...
    static QuerySet* getFromFile(const char *, unsigned int);
    QuerySet(unsigned int count, unsigned int attr_count) {
        query_count = count;
        attrs_count = attr_count;
        queries = new char*[query_count];
        for (int i = 0; i < query_count; ++i) {
            queries[i] = new char [attr_count];
        }
    }
};

QuerySet* QuerySet::getFromFile(const char *filename, unsigned int attr_count) {
    int query_count = 0;
    ...
    auto query_set = new QuerySet(query_count, attr_count);
